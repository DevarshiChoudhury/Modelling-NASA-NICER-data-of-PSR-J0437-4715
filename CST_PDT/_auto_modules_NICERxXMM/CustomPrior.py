""" Prior module for X-PSI CST+PDT modelling of NICER & XMM PSR J0437-4715 event data. """
...
class CustomPrior(xpsi.Prior):
    ...
    __derived_names__ = ['compactness', 'N_H_e20_units', 'p__phase_shift_shifted', 's__phase_shift_shifted']
    ...
    def __call__(self, p = None):
        ...
        if self._overlap(ref, 's', 'p', 'super', 'super', 0.5, 0.0):
            if not self._overlap(ref, 's', 'p', 'super', 'omit', 0.5, 0.0, superset='p'):
                return -np.inf

        if self._overlap(ref, 's', 'p', 'cede', 'super', 0.5, 0.0):
            if not self._overlap(ref, 's', 'p', 'cede', 'omit', 0.5, 0.0, superset='p'):
                return -np.inf
    ...
    def transform(self, p, **kwargs):
        ...
        # DEVARSHI: N_H in units of 10^20 cm^-2
        p += [ref['neutral_hydrogen_column_density']*0.4]

        # DEVARSHI: shifting phase shift intervals
        for phase_shift in ['p__phase_shift', 's__phase_shift']:
            if ref[phase_shift] < 0.0:
