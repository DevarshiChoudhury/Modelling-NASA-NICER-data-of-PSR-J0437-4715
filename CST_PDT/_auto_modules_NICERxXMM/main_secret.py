""" Main module for NICER x XMM PSR J0437-4715 <- X-PSI 2.1.1 CST+PDT"""
import os
import argparse
import re
...
class GenerateConfigAction(argparse.Action):
    ...
    def __call__(self, parser, namespace, values, option_string=None):
        ...
        with open('./config_NICERxXMM.ini', 'w') as file:
...
parser = ArgumentParserCustom(
    description="""
    Main module for X-PSI CST+PDT modelling of NICER & XMM PSR J0437-4715 event data.
...
parser.add_argument('--MOS1-exposure-time',
                    type=float,
                    help='MOS1 exposure time in seconds.',
                    comment_line_above='MOS1 configuration flags')

parser.add_argument('--MOS1-count-matrix-path', type=str, help='Absolute or relative path to MOS1 channel-phase count matrix. If the data is a spectrum (phase-averaged), then the file must contain a vector. This path is written to if the file does not exist by processing the event files.')
parser.add_argument('--MOS1-count-matrix-type', type=str, default='double', help='MOS1 count matrix NumPy data type.',
                    comment=True)
parser.add_argument('--MOS1-event-path', type=str, help='Absolute or relative path to MOS1 event list file.')
parser.add_argument('--MOS1-number-phase-bins', type=int, help='Number of phases bins for binning MOS1 event list file.')
parser.add_argument('--MOS1-event-file-channel-column', type=int, default=1, help='Channel column in MOS1 event list file.',
                    comment=True)
parser.add_argument('--MOS1-event-file-phase-column', type=int, default=2, help='Phase column in MOS1 event list file.',
                    comment=True)
parser.add_argument('--MOS1-event-file-skiprows', type=int, default=3, help='Number of top rows to skip when loading MOS1 event list file.',
                    comment=True)
parser.add_argument('--MOS1-events-in-eV', action='store_true', help='MOS1 event list file lists events by energy in eV?',
                    comment=True)

parser.add_argument('--MOS1-rsp-path', type=str, help='Absolute or relative path to MOS1 RSP file.',
                    comment_line_above='rule')
parser.add_argument('--MOS1-effective-area-scaling-factor', type=str, default='1.0',
                    help='Factor by which to scale the nominal effective area model, as a mathematical expression, e.g., a ratio of integers such as 51.0/52.0.',
                    comment=True)
parser.add_argument('--MOS1-rsp-skiprows', type=int, default=3,
                    help='Number of header rows to skip when loading RSP file.',
                    comment=True)
parser.add_argument('--MOS1-rsp-usecol', type=int, default=-1,
                    help='Column (zero-indexed) containing the flattened redistribution matrix elements in the RSP file.',
                    comment=True)

parser.add_argument('--MOS1-channels-path', type=str, help='Absolute or relative path to MOS1 channel-energy mapping file.',
                    comment_line_above='rule')
parser.add_argument('--MOS1-channel-energies-skiprows', type=int, default=0,
                    help='Number of header rows to skip when loading channel-energy mapping file.',
                    comment=True)
parser.add_argument('--MOS1-channel-energies-low-column', type=int, default=0,
                    help='Column (zero-indexed) containing the low energy edges in the channel-energy mapping file.',
                    comment=True)

parser.add_argument('--MOS1-input-bounds',
                    type=int,
                    nargs=2,
                    help='MOS1 bounding input energy intervals of instrument response submatrix for use with NumPy slice notation.',
                    comment_line_above='rule')

parser.add_argument('--MOS1-channel-bounds',
                    type=int,
                    nargs=2,
                    help='MOS1 bounding channels of instrument response submatrix for use with NumPy slice notation.')

parser.add_argument('--MOS1-energy-independent-effective-area-scaling-factor-bounds',
                    type=str,
                    nargs=2,
                    action=CompileAction,
                    help='Bounds for MOS1 energy-independent effective area scaling factor parameter. If no bounds are given (``None``), and no value is given (``None``), the parameter value is fixed at unity, and the instrument response model is locked to the nominal response model (unless a custom model is implemented).',
                    comment=True,
                    comment_line_above='rule')

parser.add_argument('--MOS1-energy-independent-effective-area-scaling-factor-prior',
                    type=str,
                    nargs='*',
                    default=['truncnorm.ppf(x, -3.0, 3.0, loc=1.0, scale=0.1)'],
                    action=NullAction,
...
parser.add_argument('--MOS1-energy-independent-effective-area-scaling-factor-value',
                    type=str,
                    action=CompileAction,
                    help='Value for MOS1 energy-independent effective area scaling parameter. Either the name of an instrument to share the parameter with, as a string, or a float. No value means the parameter is free, whilst a value means the parameter is fixed (and the prior may need to be modified manually). If you want the parameter to be derived from other parameters in a complex way, manual modification of the main module is necessary, but we support functions of one parameter in the configuration file. If the parameter is derived from one other parameter, e.g., the temperature of a hot region is derived from the temperature of the other hot region, then the value needs to be written using the following template: lambda x: f(x), "parameter", "space". In this template: f(x) is a function of the parameter x from which the value is derived; "parameter" is the name of the parameter x as a string; and "space" is an object in the global namespace, with name written as a string, which is a (sub)space of parameters from which the current value of parameter x can be accessed via getitem magic using "parameter".',
                    comment=True,
                    empty_lines_below=2)

parser.add_argument('--MOS1-phase-shift-bounds',
                    type=str,
                    nargs=2,
                    action=CompileAction,
                    help='Bounds for MOS1 phase-shift parameter. If no bounds are given (``None``), and no value is given (``None``), the parameter value is fixed at zero, and is therefore locked to the phase of the signal specified by the hot region phases. For one phase-resolving instrument, this default behaviour is advised, and additional phase-resolving instruments can in principle have a different fixed, derived, or free phase-shift parameter. For instruments that phase-average, the phase-shift can be arbitrarily fixed or derived, but not free because the likelihood is not a function of it.',
                    comment=True,
                    comment_line_above='rule')

parser.add_argument('--MOS1-phase-shift-value',
                    type=str,
                    action=CompileAction,
                    help='Value for MOS1 phase-shift parameter. Either the name of an instrument to share the parameter with, as a string, or a float. No value means the parameter is free, whilst a value means the parameter is fixed (and the prior may need to be modified manually). If you want the parameter to be derived from other parameters in a complex way, manual modification of the main module is necessary, but we support functions of one parameter in the configuration file. If the parameter is derived from one other parameter, e.g., the temperature of a hot region is derived from the temperature of the other hot region, then the value needs to be written using the following template: lambda x: f(x), "parameter", "space". In this template: f(x) is a function of the parameter x from which the value is derived; "parameter" is the name of the parameter x as a string; and "space" is an object in the global namespace, with name written as a string, which is a (sub)space of parameters from which the current value of parameter x can be accessed via getitem magic using "parameter".',
                    comment=True,
                    empty_lines_below=2)

parser.add_argument('--MOS1-background-prior-support-path', type=str, help='Absolute or relative path to MOS1 background prior support file. The channel-by-channel lower count-rate limits in the zeroth column, and the upper count-rate limits in the first column. The channels must already match the data.',
                    comment=True,
                    comment_line_above='rule')
parser.add_argument('--MOS1-background-skiprows', type=int, default=0, help='Number of top rows to skip when loading MOS1 background file (prior support or spectrum file).',
                    comment=True)

parser.add_argument('--MOS1-background-path', type=str, help='Absolute or relative path to MOS1 background spectrum file (for imaging telescope).',
                    comment=True)
parser.add_argument('--MOS1-background-usecol', type=int, help='Column to use when loading MOS1 background spectrum file (for imaging telescope).',
                    comment=True)
parser.add_argument('--MOS1-background-prior-support-half-width', type=float, help='MOS1 background prior support half-width (for imaging telescope). The half-width is in units of standard deviation of background count number per instrument channel.',
                    comment=True)
parser.add_argument('--MOS1-background-exposure-time',
                    type=float,
                    help='MOS1 background exposure time in seconds (for imaging telescope).',
                    comment=True)
parser.add_argument('--MOS1-background-scaling-factor',
                    type=str,
                    help='MOS1 background scaling factor, nominally the ratio of on-source CCD extraction area to background CCD extraction area (ideally on same CCD) for imaging telescope. Supply an expression for evaluation by the ``eval(...)`` builtin function.',
                    comment=True,
                    empty_lines_below=2)

parser.add_argument('--MOS2-exposure-time',
                    type=float,
                    help='MOS2 exposure time in seconds.',
                    comment_line_above='MOS2 configuration flags')

parser.add_argument('--MOS2-count-matrix-path', type=str, help='Absolute or relative path to MOS2 channel-phase count matrix. If the data is a spectrum (phase-averaged), then the file must contain a vector. This path is written to if the file does not exist by processing the event files.')
parser.add_argument('--MOS2-count-matrix-type', type=str, default='double', help='MOS2 count matrix NumPy data type.',
                    comment=True)
parser.add_argument('--MOS2-event-path', type=str, help='Absolute or relative path to MOS2 event list file.')
parser.add_argument('--MOS2-number-phase-bins', type=int, help='Number of phases bins for binning MOS2 event list file.')
parser.add_argument('--MOS2-event-file-channel-column', type=int, default=1, help='Channel column in MOS2 event list file.',
                    comment=True)
parser.add_argument('--MOS2-event-file-phase-column', type=int, default=2, help='Phase column in MOS2 event list file.',
                    comment=True)
parser.add_argument('--MOS2-event-file-skiprows', type=int, default=3, help='Number of top rows to skip when loading MOS2 event list file.',
                    comment=True)
parser.add_argument('--MOS2-events-in-eV', action='store_true', help='MOS2 event list file lists events by energy in eV?',
                    comment=True)
parser.add_argument('--MOS2-rsp-path', type=str, help='Absolute or relative path to MOS2 RSP file.',
                    comment_line_above='rule')
parser.add_argument('--MOS2-effective-area-scaling-factor', type=str, default='1.0',
                    help='Factor by which to scale the nominal effective area model, as a mathematical expression, e.g., a ratio of integers such as 51.0/52.0.',
                    comment=True)
parser.add_argument('--MOS2-rsp-skiprows', type=int, default=3,
                    help='Number of header rows to skip when loading RSP file.',
                    comment=True)
parser.add_argument('--MOS2-rsp-usecol', type=int, default=-1,
                    help='Column (zero-indexed) containing the flattened redistribution matrix elements in the RSP file.',
                    comment=True)

parser.add_argument('--MOS2-channels-path', type=str, help='Absolute or relative path to MOS2 channel-energy mapping file.',
                    comment_line_above='rule')
parser.add_argument('--MOS2-channel-energies-skiprows', type=int, default=0,
                    help='Number of header rows to skip when loading channel-energy mapping file.',
                    comment=True)
parser.add_argument('--MOS2-channel-energies-low-column', type=int, default=0,
                    help='Column (zero-indexed) containing the low energy edges in the channel-energy mapping file.',
                    comment=True)

parser.add_argument('--MOS2-input-bounds',
                    type=int,
                    nargs=2,
                    help='MOS2 bounding input energy intervals of instrument response submatrix for use with NumPy slice notation.',
                    comment_line_above='rule')

parser.add_argument('--MOS2-channel-bounds',
                    type=int,
                    nargs=2,
                    help='MOS2 bounding channels of instrument response submatrix for use with NumPy slice notation.')

parser.add_argument('--MOS2-energy-independent-effective-area-scaling-factor-bounds',
                    type=str,
                    nargs=2,
                    action=CompileAction,
                    help='Bounds for MOS2 energy-independent effective area scaling factor parameter. If no bounds are given (``None``), and no value is given (``None``), the parameter value is fixed at unity, and the instrument response model is locked to the nominal response model (unless a custom model is implemented).',
                    comment=True,
                    comment_line_above='rule')

parser.add_argument('--MOS2-energy-independent-effective-area-scaling-factor-prior',
                    type=str,
                    nargs='*',
                    default=['truncnorm.ppf(x, -3.0, 3.0, loc=1.0, scale=0.1)'],
                    action=NullAction,
...
parser.add_argument('--MOS2-energy-independent-effective-area-scaling-factor-value',
                    type=str,
                    action=CompileAction,
                    help='Value for MOS2 energy-independent effective area scaling parameter. Either the name of an instrument to share the parameter with, as a string, or a float. No value means the parameter is free, whilst a value means the parameter is fixed (and the prior may need to be modified manually). If you want the parameter to be derived from other parameters in a complex way, manual modification of the main module is necessary, but we support functions of one parameter in the configuration file. If the parameter is derived from one other parameter, e.g., the temperature of a hot region is derived from the temperature of the other hot region, then the value needs to be written using the following template: lambda x: f(x), "parameter", "space". In this template: f(x) is a function of the parameter x from which the value is derived; "parameter" is the name of the parameter x as a string; and "space" is an object in the global namespace, with name written as a string, which is a (sub)space of parameters from which the current value of parameter x can be accessed via getitem magic using "parameter".',
                    comment=True,
                    empty_lines_below=2)

parser.add_argument('--MOS2-phase-shift-bounds',
                    type=str,
                    nargs=2,
                    action=CompileAction,
                    help='Bounds for MOS2 phase-shift parameter. If no bounds are given (``None``), and no value is given (``None``), the parameter value is fixed at zero, and is therefore locked to the phase of the signal specified by the hot region phases. For one phase-resolving instrument, this default behaviour is advised, and additional phase-resolving instruments can in principle have a different fixed, derived, or free phase-shift parameter. For instruments that phase-average, the phase-shift can be arbitrarily fixed or derived, but not free because the likelihood is not a function of it.',
                    comment=True,
                    comment_line_above='rule')

parser.add_argument('--MOS2-phase-shift-value',
                    type=str,
                    action=CompileAction,
                    help='Value for MOS2 phase-shift parameter. Either the name of an instrument to share the parameter with, as a string, or a float. No value means the parameter is free, whilst a value means the parameter is fixed (and the prior may need to be modified manually). If you want the parameter to be derived from other parameters in a complex way, manual modification of the main module is necessary, but we support functions of one parameter in the configuration file. If the parameter is derived from one other parameter, e.g., the temperature of a hot region is derived from the temperature of the other hot region, then the value needs to be written using the following template: lambda x: f(x), "parameter", "space". In this template: f(x) is a function of the parameter x from which the value is derived; "parameter" is the name of the parameter x as a string; and "space" is an object in the global namespace, with name written as a string, which is a (sub)space of parameters from which the current value of parameter x can be accessed via getitem magic using "parameter".',
                    comment=True,
                    empty_lines_below=2)

parser.add_argument('--MOS2-background-prior-support-path', type=str, help='Absolute or relative path to MOS2 background prior support file. The channel-by-channel lower count-rate limits in the zeroth column, and the upper count-rate limits in the first column. The channels must already match the data.',
                    comment=True,
                    comment_line_above='rule')
parser.add_argument('--MOS2-background-skiprows', type=int, default=0, help='Number of top rows to skip when loading MOS2 background file (prior support or spectrum file).',
                    comment=True)

parser.add_argument('--MOS2-background-path', type=str, help='Absolute or relative path to MOS2 background spectrum file (for imaging telescope).',
                    comment=True)
parser.add_argument('--MOS2-background-usecol', type=int, help='Column to use when loading MOS2 background spectrum file (for imaging telescope).',
                    comment=True)
parser.add_argument('--MOS2-background-prior-support-half-width', type=float, help='MOS2 background prior support half-width (for imaging telescope). The half-width is in units of standard deviation of background count number per instrument channel.',
                    comment=True)
parser.add_argument('--MOS2-background-exposure-time',
                    type=float,
                    help='MOS2 background exposure time in seconds (for imaging telescope).',
                    comment=True)
parser.add_argument('--MOS2-background-scaling-factor',
                    type=str,
                    help='MOS2 background scaling factor, nominally the ratio of on-source CCD extraction area to background CCD extraction area (ideally on same CCD) for imaging telescope. Supply an expression for evaluation by the ``eval(...)`` builtin function.',
                    comment=True,
                    empty_lines_below=2)

parser.add_argument('--AGN-path', type=str, help='Absolute or relative path to AGN estimates',
                    comment_line_above='rule')
...
if __name__ == '__main__':
    ...
    if xpsi._verbose:
        print('Parsing configuration file...')
    args, _ = parser.parse_known_args(['@./config_NICERxXMM.ini'])
    if xpsi._verbose:
        print('Configuration file parsed.')
...
if args.XTI_energy_independent_effective_area_scaling_factor_value is not None:
    if eval(args.XTI_energy_independent_effective_area_scaling_factor_value) in ['XTI', 'MOS1', 'MOS2']:
...
if args.XTI_background_prior_support_path:
    ...
    # DEVARSHI: Added the following block to account
    # for statistical and systematic error
    #-------------------------------------------------------------#
    ERR_STAT = np.loadtxt(args.XTI_background_path,
                          skiprows=args.XTI_background_skiprows,
                          usecols=args.XTI_background_usecol+1,
                          dtype=np.double)[XTI.instrument.channels]
    spectrum_norm = spectrum[0:170]
    ERR_SYS = spectrum*0.018/spectrum_norm.sum()
    err = np.sqrt(ERR_STAT**2.+ERR_SYS**2.)
    #-------------------------------------------------------------#

    # DEVARSHI: Loading 3C50 AGN estimate
    agn_3c50 = fits.open(args.AGN_path)
    agn_3c50 = np.array(agn_3c50[1].data[30:300].tolist())

    # DEVARSHI: modified the support to take in the error calculated in the block above
    support = np.zeros((len(spectrum), 2), dtype=np.double)
    support[:,0] = spectrum - 3.0*err + agn_3c50[:,1]/2.0 #lower limit
    support[:,0] = savgol_filter(support[:,0], 17, 5) #smoothening lower limit
    support[support[:,0] < 0.0, 0] = 0.0
    support[:,1] = spectrum + 3.0*err + 2.0*agn_3c50[:,1] #upper limit
    support[:,1] = savgol_filter(support[:,1], 17, 5) #smoothening upper limit
...
if args.XTI_phase_shift_value is not None:
    if eval(args.XTI_phase_shift_value) in ['XTI', 'MOS1', 'MOS2']:
...
if args.MOS1_energy_independent_effective_area_scaling_factor_value is not None:
    if eval(args.MOS1_energy_independent_effective_area_scaling_factor_value) in ['XTI', 'MOS1', 'MOS2']:
        values = dict(energy_independent_effective_area_scaling_factor = derived_parameter(lambda x: x,
                                              'energy_independent_effective_area_scaling_factor',
                                              eval(args.MOS1_energy_independent_effective_area_scaling_factor_value) + '.instrument'))
    else:
        values = dict(energy_independent_effective_area_scaling_factor = parse_value(args.MOS1_energy_independent_effective_area_scaling_factor_value))
else:
    values = {}

bounds = dict(energy_independent_effective_area_scaling_factor = parse_bounds(args.MOS1_energy_independent_effective_area_scaling_factor_bounds,
                                   args.MOS1_energy_independent_effective_area_scaling_factor_value,
                                   default_to_free = False))

# DEVARSHI: modified according to changes made in CustomInstrument
MOS1.instrument = CustomInstrument.MOS1(bounds=bounds,
                                  values=values,
                                  RSP=args.MOS1_rsp_path,
                                  channel_edges=args.MOS1_channels_path,
                                  max_input=args.MOS1_input_bounds[1],
                                  max_channel=args.MOS1_channel_bounds[1],
                                  min_input=args.MOS1_input_bounds[0],
                                  min_channel=args.MOS1_channel_bounds[0],
                                  effective_area_scaling_factor=eval(args.MOS1_effective_area_scaling_factor),
                                  RSP_skiprows=args.MOS1_rsp_skiprows,
                                  RSP_usecol=args.MOS1_rsp_usecol,
                                  channel_edges_skiprows=args.MOS1_channel_energies_skiprows)

try:
    # DEVARSHI: modified to take account for headers
    counts = np.loadtxt(args.MOS1_count_matrix_path,
                        skiprows=3,
                        usecols=1,
                        dtype=np.double)[MOS1.instrument.channels]
except ValueError:
    MOS1.data = xpsi.Data.bin__event_list(args.MOS1_event_path,
                                         channels=MOS1.instrument.channels,
                                         phases=np.linspace(0.0, 1.0, args.MOS1_number_phase_bins + 1),
                                         channel_column=args.MOS1_event_file_channel_column,
                                         phase_column=args.MOS1_event_file_phase_column if args.MOS1_number_phase_bins > 1 else None,
                                         phase_averaged=True if args.MOS1_number_phase_bins == 1 else False,
                                         channel_edges=MOS1.instrument.channel_edges,
                                         skiprows=args.MOS1_event_file_skiprows,
                                         eV=True if args.MOS1_events_in_eV else False,
                                         dtype=getattr(np, args.MOS1_count_matrix_type),
                                         first=0,
                                         last=len(MOS1.instrument.channels) - 1,
                                         exposure_time=args.MOS1_exposure_time)

    np.savetxt(args.MOS1_event_path.replace('.txt','_converted_to_counts.txt'), MOS1.data.counts)
    print('Counts file saved as: '+args.MOS1_event_path.replace('.txt','_converted_to_counts.txt'))
    print('Update configuration file to take in counts file to save computation time.')
else:
    if counts.ndim == 1: # DEVARSHI: should enter this for current file setup
        counts = counts.reshape(-1,1)

    MOS1.data = xpsi.Data(counts,
                           channels=MOS1.instrument.channels,
                           phases=np.linspace(0.0, 1.0, args.MOS1_number_phase_bins + 1),
                           first=0,
                           last=len(MOS1.instrument.channels) - 1,
...
if args.MOS1_background_prior_support_path:
    support = np.loadtxt(args.MOS1_background_path,
                         skiprows=args.MOS1_background_skiprows,
                         dtype=np.double)

elif args.MOS1_background_path:
    spectrum = np.loadtxt(args.MOS1_background_path,
                          skiprows=args.MOS1_background_skiprows,
                          usecols=args.MOS1_background_usecol,
                          dtype=np.double)[MOS1.instrument.channels]

    support = np.zeros((len(spectrum), 2), dtype=np.double)
    support[:,0] = spectrum - args.MOS1_background_prior_support_half_width * np.sqrt(spectrum)
    support[support[:,0] < 0.0, 0] = 0.0
    support[:,1] = spectrum + args.MOS1_background_prior_support_half_width * np.sqrt(spectrum)
    ...
    support *= (MOS1.data.exposure_time / args.MOS1_background_exposure_time) * float(eval(args.MOS1_background_scaling_factor)) # exposure ratio * scaling

    support /= MOS1.data.exposure_time # need count rate, so divide by exposure time
else:
    support = None

MOS1.background = None

if args.MOS1_phase_shift_value is not None:
    if eval(args.MOS1_phase_shift_value) in ['XTI', 'MOS1', 'MOS2']:
        values = dict(phase_shift = derived_parameter(lambda x: x,
                                                'phase_shift',
                                                eval(args.MOS1_phase_shift_value) + '.signal'))
    else:
        values = dict(phase_shift = parse_value(args.MOS1_phase_shift_value))
else:
    values = {}

bounds = dict(phase_shift = parse_bounds(args.MOS1_phase_shift_bounds,
                                         args.MOS1_phase_shift_value,
                                         default_to_free=False))

MOS1.signal = CustomSignal(data = MOS1.data,
                          instrument = MOS1.instrument,
                          interstellar = interstellar,
                          background = MOS1.background,
                          cache = False if __name__ == '__main__' else True,
                          bounds = bounds,
                          values = values,
                          workspace_intervals = 1000,
...
#DEVARSHI: Added the following code block
class derive(Derive):
    global MOS1
...
if args.MOS2_energy_independent_effective_area_scaling_factor_value is not None:
    if eval(args.MOS2_energy_independent_effective_area_scaling_factor_value) in ['XTI', 'MOS1', 'MOS2']:
        values = dict(energy_independent_effective_area_scaling_factor = derived_parameter(lambda x: x,
                                              'energy_independent_effective_area_scaling_factor',
                                              eval(args.MOS2_energy_independent_effective_area_scaling_factor_value) + '.instrument'))
    else:
        values = dict(energy_independent_effective_area_scaling_factor = parse_value(args.MOS2_energy_independent_effective_area_scaling_factor_value))
else:
    values = {}

bounds = dict(energy_independent_effective_area_scaling_factor = parse_bounds(args.MOS2_energy_independent_effective_area_scaling_factor_bounds,
                                   args.MOS2_energy_independent_effective_area_scaling_factor_value,
                                   default_to_free = False))

# DEVARSHI: modified according to changes made in CustomInstrument
MOS2.instrument = CustomInstrument.MOS2(bounds={'alpha': None},
                                  values={'alpha': derive()},
                                  RSP=args.MOS2_rsp_path,
                                  channel_edges=args.MOS2_channels_path,
                                  max_input=args.MOS2_input_bounds[1],
                                  max_channel=args.MOS2_channel_bounds[1],
                                  min_input=args.MOS2_input_bounds[0],
                                  min_channel=args.MOS2_channel_bounds[0],
                                  effective_area_scaling_factor=eval(args.MOS2_effective_area_scaling_factor),
                                  RSP_skiprows=args.MOS2_rsp_skiprows,
                                  RSP_usecol=args.MOS2_rsp_usecol,
                                  channel_edges_skiprows=args.MOS2_channel_energies_skiprows)

try:
    # DEVARSHI: modified to take account for headers
    counts = np.loadtxt(args.MOS2_count_matrix_path,
                        skiprows=3,
                        usecols=1,
                        dtype=np.double)[MOS2.instrument.channels]
except ValueError:
    MOS2.data = xpsi.Data.bin__event_list(args.MOS2_event_path,
                                         channels=MOS2.instrument.channels,
                                         phases=np.linspace(0.0, 1.0, args.MOS2_number_phase_bins + 1),
                                         channel_column=args.MOS2_event_file_channel_column,
                                         phase_column=args.MOS2_event_file_phase_column if args.MOS2_number_phase_bins > 1 else None,
                                         phase_averaged=True if args.MOS2_number_phase_bins == 1 else False,
                                         channel_edges=MOS2.instrument.channel_edges,
                                         skiprows=args.MOS2_event_file_skiprows,
                                         eV=True if args.MOS2_events_in_eV else False,
                                         dtype=getattr(np, args.MOS2_count_matrix_type),
                                         first=0,
                                         last=len(MOS2.instrument.channels) - 1,
                                         exposure_time=args.MOS2_exposure_time)

    np.savetxt(args.MOS2_event_path.replace('.txt','_converted_to_counts.txt'), MOS2.data.counts)
    print('Counts file saved as: '+args.MOS2_event_path.replace('.txt','_converted_to_counts.txt'))
    print('Update configuration file to take in counts file to save computation time.')
else:
    if counts.ndim == 1: # DEVARSHI: should enter this for current file setup
        counts = counts.reshape(-1,1)

    MOS2.data = xpsi.Data(counts,
                           channels=MOS2.instrument.channels,
                           phases=np.linspace(0.0, 1.0, args.MOS2_number_phase_bins + 1),
                           first=0,
                           last=len(MOS2.instrument.channels) - 1,
...
if args.MOS2_background_prior_support_path:
    support = np.loadtxt(args.MOS2_background_path,
                         skiprows=args.MOS2_background_skiprows,
                         dtype=np.double)

elif args.MOS2_background_path:
    spectrum = np.loadtxt(args.MOS2_background_path,
                          skiprows=args.MOS2_background_skiprows,
                          usecols=args.MOS2_background_usecol,
                          dtype=np.double)[MOS2.instrument.channels]

    support = np.zeros((len(spectrum), 2), dtype=np.double)
    support[:,0] = spectrum - args.MOS2_background_prior_support_half_width * np.sqrt(spectrum)
    support[support[:,0] < 0.0, 0] = 0.0
    support[:,1] = spectrum + args.MOS2_background_prior_support_half_width * np.sqrt(spectrum)
    ...
    support *= (MOS2.data.exposure_time / args.MOS2_background_exposure_time) * float(eval(args.MOS2_background_scaling_factor)) # exposure ratio * scaling

    support /= MOS2.data.exposure_time # need count rate, so divide by exposure time
else:
    support = None

MOS2.background = None

if args.MOS2_phase_shift_value is not None:
    if eval(args.MOS2_phase_shift_value) in ['XTI', 'MOS1', 'MOS2']:
        values = dict(phase_shift = derived_parameter(lambda x: x,
                                                'phase_shift',
                                                eval(args.MOS2_phase_shift_value) + '.signal'))
    else:
        values = dict(phase_shift = parse_value(args.MOS2_phase_shift_value))
else:
    values = {}

bounds = dict(phase_shift = parse_bounds(args.MOS2_phase_shift_bounds,
                                         args.MOS2_phase_shift_value,
                                         default_to_free=False))

MOS2.signal = CustomSignal(data = MOS2.data,
                          instrument = MOS2.instrument,
                          interstellar = interstellar,
                          background = MOS2.background,
                          cache = False if __name__ == '__main__' else True,
                          bounds = bounds,
                          values = values,
                          workspace_intervals = 1000,
...
p_maxL_noMM = [0.129411922169896965E+01,
               0.973433968225189616E+01,
              -0.737821733079871422E+00,
               0.454721481685848694E+00,
               0.236079772576513164E+00,
               0.334816783808590868E+00,
               0.610462799056635674E+01,
               0.107569926727370882E+00,
               0.483892876405889849E+00,
               0.235233126706664386E+01,
               0.288944505150451163E-01,
               0.624794937107346726E+01,
               0.258886251134123269E+01,
               0.218917562849968522E+00,
              -0.467263118260987653E-01,
               0.580485877534490413E+01,
               0.106259766593164851E+01,
               0.409415544749387322E-01,
               0.995664185072263841E+00]

p_maxL_MM = [0.127795550354423271E+01,
             0.943088369259966441E+01,
            -0.736171102603722871E+00,
             0.455642089788294458E+00,
             0.247666183452118022E+00,
             0.311278797398065021E+00,
             0.611772790211381867E+01,
             0.120769662432576960E+00,
             0.484201420274178340E+00,
             0.233971145770280886E+01,
             0.294576575110424288E-01,
             0.625306727310089450E+01,
             0.257251379496257560E+01,
             0.220973184278650953E+00,
            -0.468386938613507553E-01,
             0.581180911002632783E+01,
             0.106984806498151563E+01,
             0.263590968971876630E-01,
             0.991940239129029289E+00]
