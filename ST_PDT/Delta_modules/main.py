""" Main module for NICER PSR J0437-4715 <- X-PSI 0.7.9 ST+PDT + NSX-H"""
...
class GenerateConfigAction(argparse.Action):
    ...
    def __call__(self, parser, namespace, values, option_string=None):
        ...
        with open('./config_Delta_SW.ini', 'w') as file:
...
parser = ArgumentParserCustom(
    description="""
    Main module for X-PSI ST+PDT + NSX-H modelling of NICER PSR J0437-4715 event data.

    You can run this module as a script and launch a sampler, optionally
    with a world of MPI processes.

    Alternate usage: mpiexec -n 4 python -m mpi4py %(prog)s [-h] @<config_Delta_SW.ini>
...
if __name__ == '__main__':
    ...
    if xpsi._verbose:
        print('Parsing configuration file...')
    args, _ = parser.parse_known_args(['@./config_Delta_SW.ini'])
    if xpsi._verbose:
        print('Configuration file parsed.')
...
try:
    ...
    np.savetxt(args.XTI_count_matrix_path, XTI.data.counts)
else:
    if counts.ndim == 1:
...
if args.XTI_background_prior_support_path:
    ...
    support[:,1] = spectrum*1000000.0 # arbitrarily high upper limit
...
p = [0.150605518001144278E+01,
     0.114348144286214044E+02,
    -0.736749605438644539E+00,
     0.943566732808559516E-02,
     0.171324170511236107E+00,
     0.384282147495501658E+00,
     0.613243041928246058E+01,
    -0.113640737543744402E+00,
     0.828885367143926799E+00,
     0.179905304579621346E-01,
     0.633208611732571036E+01,
     0.182560967844677147E+01,
     0.135361317980593721E+01,
    -0.529276103170523982E+00,
     0.531977181740149518E+01,
     0.103974823859403864E+01,
     0.588854198018501779E-01]

likelihood.check(None, [-38386.742345], 1.0e-8,
                 physical_points=[p])
