""" Main module for NICER PSR J0437-4715 <- X-PSI 2.1.0 ST-U"""
import os
import argparse
import re
...
parser = ArgumentParserCustom(
    description="""
    Main module for X-PSI ST-U modelling of NICER PSR J0437-4715 event data.
...
parser.add_argument('--AGN-path', type=str, help='Absolute or relative path to AGN estimates',
                    comment_line_above='rule')
parser.add_argument('--AGN-exposure', type=float, help='AGN exposure in seconds')
...
if args.XTI_background_prior_support_path:
    ...
    # Accounting for statistical and systematic error associated with BKG estimates
    ...
    spectrum_smooth = savgol_filter(spectrum, 17, 5)

    # Loading Delta AGN estimate
    agn_spec = np.loadtxt(args.AGN_path)
    agn_spec_rate = agn_spec/args.AGN_exposure

    support = np.zeros((len(spectrum_smooth), 2), dtype=np.double)
    support[:,0] = spectrum_smooth - 3.0*err + agn_spec_rate[0:300-30,1]/2.0 #lower limit
    support[support[:,0] < 0.0, 0] = 0.0
    support[:,1] = spectrum_smooth + 3.0*err + 2.0*agn_spec_rate[0:300-30,1] #upper limit
...
p_maxL=[0.146803924288062815E+01,
        0.657493441442417392E+01,
        -0.737597022105697442E+00,
        -0.174699094093465346E-01,
        0.551345374997461835E+00,
        0.535166036389095101E+00,
        0.598003461022076177E+01,
        0.499222880492094623E+00,
        0.111834669447140933E+01,
        0.359045034782806832E-01,
        0.645698383335816217E+01,
        0.878014220463900430E+00,
        0.133920653863495231E+01]
