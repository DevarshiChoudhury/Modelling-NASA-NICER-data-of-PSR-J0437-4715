...
parser.add_argument('--NICER-background-path', type=str, help=_prfx + _instr + 'background spectrum file.')
...
if __name__ == '__main__':
    ...
    args = parser.parse_args(['@./config_Delta.ini','--NICER'])
...
if not args.NICER_rsp_path:
    args.NICER_rsp_path = input('Specify the NICER RSP file path: ')
...
interstellar = CustomInterstellar.from_SWG(args.attenuation_path,
                                    bounds = dict(column_density = (0.01,5.0)))

signals = [[],]

alpha_bounds = dict(alpha = (0.1, 1.9))

radio_inc      = np.radians(137.485)
radio_inc_std  = np.radians(0.038)
radio_mass     = 1.411
radio_mass_std =  0.056
DISTANCE     = 0.15669
...
BACKSCAl_BKG_MOS1 =  1.109E+01
BACKSCAL_BKG_MOS2 =  1.170E+01
...
if args.NICER:
    ...
    NICER.instrument = CustomInstrument.NICER_XTI(bounds = alpha_bounds,
                                          ...
                                          prefix = 'XTI')
    spectrum = np.loadtxt(args.NICER_background_path,
                          skiprows=0,
                          usecols=2,
                          dtype=np.double)[minCH_NICER-30:maxCH_NICER-30]

    support = np.zeros((len(spectrum), 2), dtype=np.double)
    support[:,0] = spectrum*0.9 #lower limit
    support[support[:,0] < 0.0, 0] = 0.0
    support[:,1] = spectrum*1000000.0 #upper limit
...
if args.XMM:
    ...
    support *=  (MOS1.data.exposure_time / exposure_time_BKG_MOS1)/BACKSCAl_BKG_MOS1
    ...
    support *= (MOS2.data.exposure_time / exposure_time_BKG_MOS2)/BACKSCAL_BKG_MOS2
...
if args.XMM and args.NICER:
    p =[1.4949909256216776, 6.701010757850769, -0.7348265225366337, 0.7484694308107467, 0.49418992687323, 0.5253861962180668, 5.121164939167022, 0.48877554050290617, 1.888875034679965, 0.11794663371491185, 5.37472766374477, 0.9451158417416874, 3.0, 1.1745594609294474, 0.9378078736228744]
    ...
    p =[1.4949909256216776, 6.701010757850769, -0.7348265225366337, 0.7484694308107467, 0.49418992687323, 0.5253861962180668, 5.121164939167022, 0.48877554050290617, 1.888875034679965, 0.11794663371491185, 5.37472766374477, 5.5, 0.9378078736228744, 1.1745594609294474] #elsewhere included
    #likelihood.check(None, [-7164.70853764], 1.0e-6,
    #                 physical_points=[p])
    # likelihood.check(None, [-178225.771], 1.0e-6,
    #                  physical_points=[p])                     

#-1.78225771e+05 | -1.78485329e+05 .....
                     
elif args.NICER:
    p =[1.4949909256216776, 6.701010757850769, -0.7348265225366337, 0.7484694308107467, 0.49418992687323, 0.5253861962180668, 5.121164939167022, 0.48877554050290617, 1.888875034679965, 0.11794663371491185, 5.37472766374477, 3.0, 0.9451158417416874, 1.1745594609294474]
...
if __name__ != '__main__':
    ...
    if args.XMM:
            names.append(['MOS1__alpha'])

    # names of derived variables of interest
    names += ['compactness',]# 'p__phase_shift_shifted', 's__phase_shift_shifted']

    mBound = np.cos(radio_inc - 10.0 *radio_inc_std)
    pBound = np.cos(radio_inc + 10.0 * radio_inc_std)
    min_cos_inc = np.min([mBound,pBound])
    max_cos_inc = np.max([mBound,pBound])
    bounds = {'mass': (radio_mass - 10.0 * radio_mass_std, radio_mass + 10.0 * radio_mass_std, ),
              'radius': (3.0 * gravradius(1.0), 16.0),
              'cos_inclination': (min_cos_inc, max_cos_inc),
    ...
    if args.NICER:
        bounds.update({'XTI__alpha': (0.1, 1.9)})
    if args.XMM:
        bounds.update({'MOS1__alpha': (0.1, 1.9)})
    ...
    if args.NICER:
        labels.update({'XTI__alpha': r"\alpha_{\rm XTI}"})
    if args.XMM:
        labels.update({'MOS1__alpha': r"\alpha_{\rm mos1}"})
    ...
    runtime_params = {'resume': False,
                      ...
                      'outputfiles_basename': './stu_NICER_run1_nlive4000_eff0.3_noCONST_noMM_noIS_tol-1',
