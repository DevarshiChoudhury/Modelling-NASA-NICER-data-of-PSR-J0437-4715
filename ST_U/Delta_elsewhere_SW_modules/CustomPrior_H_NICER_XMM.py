...
radio_incl = np.radians(137.485)
radio_incl_std = np.radians(0.038)
    ...
    * p[12] = hydrogen column density (10^20 cm^-2)
    ...
    def inverse_sample(self, hypercube=None):
        ...
        try:
            ...
            ref['XTI__alpha'] = truncnorm.ppf(hypercube[idx], -3.0, 3.0,
                                          loc=1.0, scale=0.104)

        idx = ref.index('mass')
        ref['mass'] = truncnorm.ppf(hypercube[idx], -10.0, 10.0,
                                    loc=radio_mass, scale=radio_mass_std)

        idx = ref.index('cos_inclination')
        _i = truncnorm.ppf(hypercube[idx], -10.0, 10.0,
                           loc=radio_incl, scale=radio_incl_std)
        ...
        try:
            ...
            ref['MOS1__alpha'] = truncnorm.ppf(hypercube[idx], -3.0, 3.0,
                                         loc=_loc,
                                         scale=_scale)
